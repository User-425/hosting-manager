...
export async function processCommand(inputStr) {
    ...
    switch (command.toLowerCase()) {
        ...
        case "ntfy": {
            if (args.length < 2) {
                console.log("Usage: ntfy <topic> <message...>");
                console.log("Example: ntfy my_alerts Server is starting!");
                break;
            }
            const ntfyTopic = args[0];
            const ntfyMessage = args.slice(1).join(" ");
            ...
            const serverUrl = serverUrlStr.endsWith('/') ? serverUrlStr.slice(0, -1) : serverUrlStr;
            ...
            try {
                const response = await fetch(`${serverUrl}/${ntfyTopic}`, {
                    method: 'POST',
                    body: ntfyMessage,
                    headers
                });
                if (response.ok) {
                    console.log(`[NTFY] Notification sent to topic "${ntfyTopic}" successfully.`);
                } else {
                    console.error(`[NTFY ERROR] Failed to send notification: ${response.status} ${response.statusText}`);
                }
            } catch (err) {
                console.error(`[NTFY ERROR] Exception: ${err.message}`);
            }
            break;
        }
